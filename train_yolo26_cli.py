...
def setup_wandb(project_name: str = "yolo26-construction", entity: str = None, config: dict = None):
    ...
    try:
        ...
        # .env 파일 로드
        load_dotenv('/home/themiraclesoft/wishket/.env')
        
        # W&B API 키 확인
        if not os.getenv('WANDB_API_KEY'):
            print("⚠️  WANDB_API_KEY가 없습니다. .env 파일을 확인하세요.")
            return None
...
def main():
    ...
    print(f"📊 설정: Epochs={args.epochs}, Batch={args.batch_size}, Device={args.device}")
    print(f"⚙️  옵티마이저: {args.optimizer} (lr={args.lr})")
    print("="*80)
    
    # 로그 디렉토리 설정
    log_dir = Path(f"/home/themiraclesoft/wishket/training_logs/{args.run_name}")
